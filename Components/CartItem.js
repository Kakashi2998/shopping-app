...
const CartItem = ({item}) => {
    ...
    return (
        <Surface style={{margin: 10, elevation: 7, overflow: 'hidden', borderRadius: 10}}>
            <View style={{flexDirection: 'row'}}>
                ...
                <View>
                    ...
                    <Title style={styles.price}>Price: {(item.price * item.qty).toFixed(2)}</Title>
                </View>
            </View>
            <View 
            style={{flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 10}}>
                ...
                <IconButton color='red' icon='delete' 
                onPress={deleteItem}/>
            </View>
        </Surface>
    );
